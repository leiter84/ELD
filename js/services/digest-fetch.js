...
export default class DigestClient {
  constructor(user, password, options = {}) {
    ...
    this.cnonceSize = isNaN(_cnonceSize) ? 8 : _cnonceSize; // cnonce length 8 as default
  ...
  addAuth(url, options) {
    ...
    const response = cryptojs
      .MD5(
        `${ha1}:${this.digest.nonce}:${ncString}:${
          this.digest.cnonce
        }:${this.digest.qop}:${ha2}`
      )
      .toString();
  ...
  async parseAuth(data) {
    ...
    this.digest.cnonce = this.makeCnonce();
    this.digest.nc++;
  }
  ...
  makeCnonce() {
    ...
    for (let i = 0; i < this.cnonceSize; ++i) {
      uid += this.nonceRaw[
        Math.floor(Math.random() * this.nonceRaw.length)
      ];
    }
    return uid;
  }
